...
class Pizza
{
    ...
    public static function VerificoPizza($sabor,$tipo)
    {
        $cadena = '';
        $retorno = false;
        $datos_productos = file_get_contents('./archivos/Pizza.json');
        $arrayPizza = json_decode($datos_productos, true);
        foreach ($arrayPizza as $pizzaArr) {
            if (strcmp($pizzaArr['sabor'], $sabor) == 0 && strcmp($pizzaArr['tipo'], $tipo) == 0) {
                $cadena = '';
                $cadena .= 'existe el sabor y el tipo';
                $retorno = true;
            }
            if (strcmp($pizzaArr['tipo'], $tipo) == 0 && strcmp($pizzaArr['sabor'], $sabor) != 0) {
                $cadena = '';
    ...
    public static function traerVentasPorFecha($fecha)
    {
        if ($fecha == '') {
            $objetoAccesoDato = AccesoDatos::dameUnObjetoAcceso();
            $consulta = $objetoAccesoDato->RetornarConsulta("SELECT * FROM venta WHERE fecha = '2021-10-20'");
            ...
            $objetoAccesoDato = AccesoDatos::dameUnObjetoAcceso();
            $consulta = $objetoAccesoDato->RetornarConsulta("SELECT SUM(cantidad) AS CantidadVendidos FROM venta WHERE fecha = :fecha ");
            $consulta->bindValue(':fecha', $fecha, PDO::PARAM_STR);
    ...
    public static function TraerVentasEntreFechas($fechaUno, $fechaDos)
    {
        ...
        $objetoAccesoDato = AccesoDatos::dameUnObjetoAcceso();
        $consulta = $objetoAccesoDato->RetornarConsulta("SELECT * FROM venta WHERE fecha BETWEEN :fechaUno AND :fechaDos ORDER BY sabor");
    ...
    public static function TraerPorUsuario($mail)
    {

        $objetoAccesoDato = AccesoDatos::dameUnObjetoAcceso();
        $consulta = $objetoAccesoDato->RetornarConsulta("SELECT * FROM venta WHERE email = :mail");
    ...
    {
        $objetoAccesoDato = AccesoDatos::dameUnObjetoAcceso();
        $consulta = $objetoAccesoDato->RetornarConsulta("DELETE FROM venta WHERE numero_pedido = :numero");
    ...
    {
        $objetoAccesoDato = AccesoDatos::dameUnObjetoAcceso();
        $consulta = $objetoAccesoDato->RetornarConsulta("SELECT * FROM venta WHERE numero_pedido = :numero");
